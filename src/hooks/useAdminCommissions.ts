...
const useAdminCommissions = () => {
    ...
    const fetchCommissions = useCallback(async () => {
        ...
        try {
            ...
            const uniqueUids = [...new Set(allDocs.map((d) => d.data().earnedBy as string))];
            ...
            const pending: PendingCommission[] = pendingSnap.docs.map((d) => {
                const data = d.data();
                return {
                    id: d.id,
                    membershipId: "",
                    recipientId: data.earnedBy as string,
                    recipientName: memberMap.get(data.earnedBy as string) ?? (data.earnedBy as string),
            ...
            const history: CommissionRecord[] = releasedSnap.docs.map((d) => {
                const data = d.data();
                return {
                    id: d.id,
                    membershipId: "",
