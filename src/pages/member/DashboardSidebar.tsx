...
export const DashboardSidebar: React.FC<Props> = ({ member, rankName, currentSection, onSectionChange, items, onLogout }) => {
    ...
    const navItems = items.filter((item) => item.id !== "profile");
    ...
    return (
        <aside
            ...
            className="border-fsc-cream-dark fixed top-0 left-0 z-30 hidden h-screen flex-col overflow-hidden border-r bg-white lg:flex"
        >
            {/* Logo */}
            ...
            {/* Member info */}
            <div className="border-fsc-cream-dark overflow-hidden border-b px-3 py-3">
                <div className="flex items-center gap-3">
                    <div className="bg-fsc-navy font-display flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm text-white">
                    ...
                    >
                        <div className="font-display text-fsc-navy truncate text-sm">
                            {member.firstName} {member.lastName}
                        </div>
                        <div className="text-fsc-stone truncate text-xs">{rankName}</div>
                    </div>
                </div>
            </div>
            ...
            <div className="border-fsc-cream-dark space-y-1 border-t p-2">
                {profileItem && renderNavButton(profileItem, navItems.length)}
